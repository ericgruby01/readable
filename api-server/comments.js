...
const defaultData = {
  ...
  "comm_hufr556893hsa": {
    id: 'comm_hufr556893hsa',
    parentId: "e3xg81wkvg",
    timestamp: 1542372202199,
    body: 'F I R S T',
  ...
  "comm_hkfldkir9897hj": {
    id: 'comm_hkfldkir9897hj',
    parentId: "e3xg81wkvg",
    timestamp: 1542372339150,
    body: 'Sorry :( I was not expecting so many downvotes... I promise to never comment "First" again.',
    author: 'Udacity Student',
    voteScore: 10,
    deleted: false,
    parentDeleted: false
...
function publishByParent (token, post) {
    return new Promise((res) => {
        let comments = getData(token)
        keys = Object.keys(comments)
        filtered_keys = keys.filter(key => comments[key].parentId === post.id)
        filtered_keys.forEach(key => comments[key].parentDeleted = false)
        res(post)
    })
}
